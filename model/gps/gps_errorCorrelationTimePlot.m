...
TIME_DELTA_MAX = 100; % (ms) max time between two timesteps when comparing
...
latError1=(abs(truth_coordinate{LAT_INDEX} - coords1(:,LAT_INDEX)) .* LAT_TO_METERS);
lonError1=(abs(truth_coordinate{LON_INDEX} - coords1(:,LON_INDEX)) .* LON_TO_METERS);
timestamp1=coords1(:,TIME_INDEX);

latError2=(abs(truth_coordinate{LAT_INDEX} - coords2(:,LAT_INDEX)) .* LAT_TO_METERS);
lonError2=(abs(truth_coordinate{LON_INDEX} - coords2(:,LON_INDEX)) .* LON_TO_METERS);
...
subplot(2,1,1)
plot(x,ylat)
 title(sprintf('Instantenous Error Comparison vs. Time'))
ylabel(sprintf('Latitude error difference (%s)',unit));

subplot(2,1,2)
plot(x,ylon)
ylabel(sprintf('Longitude error difference (%s)',unit));
